		...
		if(isset($_POST["reopenSale"])){
			...
			if($productChange){
				...
				foreach ($products as $key => $value) {
					...
					$newSales = ProductsModel::UpdateProductModel($tableProducts, $item1a, $value1a, $value_1);

					$item1b = "stock";
					$value1b = $value["quantity"] + $getProduct["stock"];

					$newStock = ProductsModel::UpdateProductModel($tableProducts, $item1b, $value1b, $value_1);
		...
		if(isset($_GET["report"])){
            ...
            */
			...
			foreach ($sales as $row => $item){
				...
				$customers = CustomerController::ShowCustomerController("id", $item["idCustomer"]);
				$staff = UserController::ShowUsersController("id", $item["idSeller"]);

			echo utf8_decode("<tr>
					<td style='border:1px solid #eee;'>".$item["code"]."</td> 
					<td style='border:1px solid #eee;'>".$customers["name"]."</td>
